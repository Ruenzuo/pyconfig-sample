...
task :package, [:config] do |task, args|
  Rake::Task["bootstrap"].invoke args[:config]
...
task :bootstrap, [:config] do |task, args|
  set_hockey_app_id config: args[:config]
  prepare_xcconfig config: args[:config]
end
...
def prepare_xcconfig(config: 'Alpha')
  system "pyconfig configs/#{config.downcase}.py --output configs/xing-config.xcconfig"
end
